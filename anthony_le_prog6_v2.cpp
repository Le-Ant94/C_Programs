...
bool is_happy_prime(int x) //Determine if a number is both happy and prime
{
    ...
    #pragma omp prarallel for shared(x) private(i) reduction(+:result)
...
int main()
{
    int happy_prime_number = 0, n = 1000000, x = 1;
    
    //Parallelize base code using reduction
    #pragma omp parallel for shared(n) private(x) reduction(+:happy_prime_number)
